...
var positionDescriptions = { "Un-Assign": "un-assign a point", "American Foxhound": "pic/FoxHound.png", "Bernedoodle": "pic/Bernedoodle.png", "Cavalier King Charles Spaniel": "pic/Charles.png", "Shih Tzu": "pic/ShihTzu.png", "American Bulldog": "pic/Bulldog.png" };
...
function loadData() {
	// Get input data
	d3.csv('data/dog_indiv_50_removed_blank.csv', function (data) { // demo50 dataset
		var loaddata = jQuery.extend(true, [], data);
		for (var i = 0; i < loaddata.length; i++) {
			loaddata[i]["Name"] = "Dog " + loaddata[i]["id"];
			delete loaddata[i]["Dog"];
...
function addHelp() {
	...
	d3.select("#helpButton").on("mouseover", function () {
		...
		ial.logging.log('help', undefined, 'HelpHover', { 'level': 'INFO', 'taskType': 'dog', 'eventType': 'help_hover', 'elapsedTime': elapsedTime, 'userId': window.localStorage.getItem("userId"), 'whichCondition': window.localStorage.getItem("whichCondition"), 'data_locations': data_locations });
