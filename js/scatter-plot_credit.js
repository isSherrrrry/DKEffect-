    ...
    circle.enter().append("circle")
        ...
        .on("mouseout", function(d) {
            ...
            if (!isDragging) {
                ...
                // get the x,y coordinates of all the points on the graph to log --1 log hover over a point
                var data_locations = [];
                ...
                ial.usermodel.incrementItemWeight(d, HOVER_WEIGHT, true, {'level': 'INFO', 'taskType': 'credit', 'eventType': 'hover', 'elapsedTime': elapsedTime, 'userId': window.localStorage.getItem("userId"), 'whichCondition': window.localStorage.getItem("whichCondition"), 'pointLocation': 'scatterplot'});
        ...
        if (activePosition != "none") {
            ...
            // get the x,y coordinates of all the points on the graph to log --2 log classification
            var data_locations = [];
            d3.select("#SC").selectAll("circle").each(function(d) {
                var pt_log = { player: d.Name, x: d.x, y: d.y, cx: +this.getAttribute("cx"), cy: +this.getAttribute("cy") };
                data_locations.push(pt_log);
            ...
            ial.usermodel.incrementItemWeight(d, CLICK_WEIGHT, true, {'level': 'INFO', 'taskType': 'credit', 'eventType': 'click', 'classification': activePosition, 'userId': window.localStorage.getItem("userId"), 'whichCondition': window.localStorage.getItem("whichCondition"), 'pointLocation': 'scatterplot'});
            //修改点
            R7Insight.log(JSON.stringify(ial.logging.peek()));
            ...
            ial.usermodel.incrementItemWeight(d, CLICK_WEIGHT, true, {'level': 'INFO', 'taskType': 'credit', 'eventType': 'click', 'classification': 'none', 'userId': window.localStorage.getItem("userId"), 'whichCondition': window.localStorage.getItem("whichCondition"), 'pointLocation': 'scatterplot'});
            R7Insight.log(JSON.stringify(ial.logging.peek()));
        ...
        .on("mouseup.dragend", function(d) {
            if (isDragging) {
                ...
                // get the x,y coordinates of all the points on the graph to log --4 dragged a point??
                var data_locations = [];
                d3.select("#SC").selectAll("circle").each(function(d) {
                    var pt_log = { player: d.Name, x: d.x, y: d.y, cx: +this.getAttribute("cx"), cy: +this.getAttribute("cy") };
                    data_locations.push(pt_log);
                ...
                ial.usermodel.incrementItemWeight(d, DRAG_WEIGHT, true, {'level': 'INFO', 'taskType': 'credit', 'eventType': 'drag', 'elapsedTime': elapsedTime, 'userId': window.localStorage.getItem("userId"), 'whichCondition': window.localStorage.getItem("whichCondition"), 'pointLocation': 'scatterplot', 'data_locations': data_locations});
                R7Insight.log(JSON.stringify(ial.logging.peek()));
        ...
        if (self.dragged) {
                    ...
                    d3.select("#"+self.dropped).selectAll("circle").data(self.dropzone[self.dropped]).enter().append("circle")
                        ...
                        .on("mouseout", function(d) {
                            ...
                            if (!isDragging && this.parentNode != null) { // make sure it hasn't been removed from the bin since mouseover began
                                ...
                                // get the x,y coordinates of all the points on the graph to log   --5 Hover in bins
                                var data_locations = [];
                                d3.select("#SC").selectAll("circle").each(function(d) {
                                    var pt_log = { player: d.Name, x: d.x, y: d.y, cx: +this.getAttribute("cx"), cy: +this.getAttribute("cy") };
                                    data_locations.push(pt_log);
                                ...
                                //delete data_location*
                                ial.usermodel.incrementItemWeight(d, HOVER_WEIGHT, true, {'level': 'INFO', 'taskType': 'credit', 'eventType': 'hover', 'elapsedTime': elapsedTime, 'userId': window.localStorage.getItem("userId"), 'whichCondition': window.localStorage.getItem("whichCondition"), 'pointLocation': whichBin, 'data_locations': data_locations});
                        ...
                        .on("dblclick", function(d) {
                            ...
                            // get the x,y coordinates of all the points on the graph to log  --6 double click to clear bin
                            var data_locations = [];
                            d3.select("#SC").selectAll("circle").each(function(d) {
                                var pt_log = { player: d.Name, x: d.x, y: d.y, cx: +this.getAttribute("cx"), cy: +this.getAttribute("cy") };
                                data_locations.push(pt_log);
                            ...
                            //delete data_location
                            ial.usermodel.incrementItemWeight(d, DOUBLE_CLICK_WEIGHT, true, {'level': 'INFO', 'taskType': 'credit', 'eventType': 'double_click', 'userId': window.localStorage.getItem("userId"), 'whichCondition': window.localStorage.getItem("whichCondition"), 'pointLocation': whichBin});
                            R7Insight.log(JSON.stringify(ial.logging.peek()));
            ...
            if (self.dropped=="XL" || self.dropped=="XH") {
                if (isDragging) {
                    ...
                    // get the x,y coordinates of all the points on the graph to log  --7 log drag into bins
                    var data_locations = [];
                    d3.select("#SC").selectAll("circle").each(function(d) {
                        var pt_log = { player: d.Name, x: d.x, y: d.y, cx: +this.getAttribute("cx"), cy: +this.getAttribute("cy") };
                        data_locations.push(pt_log);
                    ...
                    // log the drag with IAL
                    ial.usermodel.incrementItemWeight(self.dragged, DRAG_WEIGHT, true, {'level': 'INFO', 'taskType': 'credit', 'eventType': 'drag', 'bin': self.dropped, 'elapsedTime': elapsedTime, 'userId': window.localStorage.getItem("userId"), 'whichCondition': window.localStorage.getItem("whichCondition"), 'data_locations': data_locations});
                    R7Insight.log(JSON.stringify(ial.logging.peek()));
...
updategraph = function(axistobeupdated,givenV,givenVchanged) {
    ...
    // get the x,y coordinates of all the points on the graph to log --8 log set attribute weight
    var data_locations = [];
    d3.select("#SC").selectAll("circle").each(function(d) {
        var pt_log = { player: d.Name, x: d.x, y: d.y, cx: +this.getAttribute("cx"), cy: +this.getAttribute("cy") };
        data_locations.push(pt_log);
    ...
    ial.usermodel.setAttributeWeightVector(V, true, {'level': 'INFO', 'taskType': 'credit', 'eventType': 'set_attribute_weight_vector_calc', 'whichAxis': axistobeupdated, 'userId': window.localStorage.getItem("userId"), 'whichCondition': window.localStorage.getItem("whichCondition"), 'data_locations': data_locations});
    // modified logging
    R7Insight.log(JSON.stringify(ial.logging.peek()));
...
updatebycb = function(axistobeupdated, selectedattr) {
    ...
    // get the x,y coordinates of all the points on the graph to log --9 log select axis
    var data_locations = [];
    d3.select("#SC").selectAll("circle").each(function(d) {
        var pt_log = { player: d.Name, x: d.x, y: d.y, cx: +this.getAttribute("cx"), cy: +this.getAttribute("cy") };
        data_locations.push(pt_log);
    ...
    // update IAL weight vector
    ial.usermodel.setAttributeWeightVector(V2, true, {'level': 'INFO', 'taskType': 'credit', 'eventType': 'set_attribute_weight_vector_select', 'whichAxis': axistobeupdated, 'userId': window.localStorage.getItem("userId"), 'whichCondition': window.localStorage.getItem("whichCondition"), 'data_locations': data_locations});
    // modified logging
    R7Insight.log(JSON.stringify(ial.logging.peek()));
