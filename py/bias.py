...
class bias(object):
    ...
    def simulate_bias_computation(self, plot_directory, time, interaction_types, min_weight, max_weight, window_method, rolling_dist, attr_weight_rolling_dist, marks, fig_num, verbose):
        ...
        for i in range(start_iter, len(self.all_logs)):
            ...
            attr_log_set, item_log_set, prev_decision = bias_util.get_logs_by_window_method(window_method, self.all_logs, self.item_logs, self.attr_logs, i, rolling_dist, attr_weight_rolling_dist, label_indices, cat_indices, prev_decision)
            ...
            if (len(item_log_set) > 0):
                ...
                cur_ac = self.compute_attribute_coverage(item_log_set, time, interaction_types, verbose)
            ...
            if (len(attr_log_set) > 0):
                cur_awc = self.compute_attribute_weight_coverage(attr_log_set, time, interaction_types, min_weight, max_weight, verbose)
            ...
            if (len(line_contents) > 0):
                ...
                last_line = '{"computing_at_interaction":"' + str(i) + '/' + str(len(self.all_logs)) + '","interaction_type":"' + self.all_logs[i]['customLogInfo']['eventType'] + '","num_interactions":"' + str(num_interactions) + '","window_method":"' + window_method + '",'
                
                if (window_method == 'fixed'):
                    last_line += '"rolling_distance":' + str(rolling_dist) + ',"attr_weight_rolling_distance":' + str(attr_weight_rolling_dist) + ','
...
#            bias_util.plot_metric(x_values, awd_metric[attribute], 'Attribute Weight Distribution (' + attribute + ') - Pilot', x_label, y_label, plot_directory, user_id + '_awd_' + attribute + '.png', marker_decisions, marks, verbose)
    ...
    def compute_attribute_coverage(self, log_set, time, interaction_types, verbose):
        log_subset = bias_util.get_log_subset(log_set, time, interaction_types)
        ...
        for attribute in self.attr_value_map.keys():
            ...
            if (self.attr_value_map[attribute]['dataType'] == 'numeric'):
                quantiles = dict()
                quantile_list = []
                for i in range(0, bias_util.num_quantiles):
                    if (i != bias_util.num_quantiles - 1):
                        quant_val = full_dist[int(math.floor((i + 1) * len(self.dataset) / bias_util.num_quantiles) - 1)]
                    else:
                ...
                for i in range(0, len(log_subset)):
                    ...
                    which_quantile = bias_util.get_quantile(quantile_list, cur_val, True)
                ...
                expected_covered_quantiles = bias_util.get_markov_expected_value(bias_util.num_quantiles, len(log_subset))
                percent_unique = covered_quantiles / expected_covered_quantiles; 
                current_log_info['attribute_vector'][attribute]['number_of_quantiles'] = bias_util.num_quantiles; 
    ...
    def compute_attribute_distribution(self, log_set, time, interaction_types, verbose):
        ...
        for attribute in self.attr_value_map.keys():
            ...
            if (self.attr_value_map[attribute]['dataType'] == 'numeric'):
                ...
                # quantize it anyway -- not used in metric computation, but can be used to visualize later
                quantiles = dict()
                quantile_list = []
                for i in range(0, bias_util.num_quantiles):
                    if (i != bias_util.num_quantiles - 1):
                ...
                current_log_info['attribute_vector'][attribute] = dict()
                current_log_info['attribute_vector'][attribute]['quantiles'] = quantile_list
                current_log_info['attribute_vector'][attribute]['quantile_distribution'] = quantiles
                ...
                current_log_info['attribute_vector'][attribute]['actual_distribution'] = full_dist
                current_log_info['attribute_vector'][attribute]['interaction_distribution'] = int_dist
    ...
    def compute_attribute_weight_coverage(self, log_set, time, interaction_types, min_weight, max_weight, verbose):
        ...
        for i in range(0, bias_util.num_quantiles):
            ...
            if (i != bias_util.num_quantiles - 1):
                quant_val = float(min_weight + (i + 1) * (max_weight - min_weight) / float(bias_util.num_quantiles))
        ...
        for i in range(0, len(weight_vector_subset)):
            ...
            for attribute in self.attr_value_map.keys():
                ...
                if (i == 0):
                    ...
                    # figure out which quantile it belongs to
                    which_quantile = bias_util.get_quantile(quantile_list, attr_weight, True)
        ...
        for attribute in self.attr_value_map.keys():
            ...
            expected_covered_quantiles = bias_util.get_markov_expected_value(bias_util.num_quantiles, change_map[attribute])
            percent_unique = covered_quantiles / expected_covered_quantiles
            current_log_info['attribute_vector'][attribute]['number_of_quantiles'] = bias_util.num_quantiles
            current_log_info['attribute_vector'][attribute]['covered_quantiles'] = covered_quantiles
            current_log_info['attribute_vector'][attribute]['expected_covered_quantiles'] = expected_covered_quantiles
    ...
    def compute_attribute_weight_distribution(self, log_set, time, interaction_types, min_weight, max_weight, verbose):
        ...
        # uniform distribution sampled N+1 times between min_weight and max weight
        uni_distr = np.random.uniform(min_weight, max_weight, len(self.dataset) - 2).tolist()
        uni_distr += [-1.0, 1.0] # make sure it contains the highest and lowest possible values
        uni_distr.sort()

        # compute the distributions of weight
        weight_distr = dict()
        for attribute in self.attr_value_map.keys():
            if (attribute == 'Rand'):
                continue # skip rand here -- weight doesn't make sense for random variable
            cur_distr = []
            for i in range(0, len(weight_vector_subset)):
                cur_distr.append(weight_vector_subset[i]['newWeight'][attribute])
            weight_distr[attribute] = cur_distr

        # compare weight distributions to baseline distribution
        # todo: if using exponential distribution, change weight_distr back to delta weight (change in weight)
        baseline_distr = uni_distr # exp_distr
        max_metric_value = 0
        for attribute in self.attr_value_map.keys():
            if (attribute == 'Rand'):
                continue # skip rand here -- weight doesn't make sense for random variable
            KS = ks_2samp(baseline_distr, weight_distr[attribute])
            if (1 - float(KS[1]) > max_metric_value):
                max_metric_value = 1 - float(KS[1])
            ...
            current_log_info['attribute_vector'][attribute]['ks'] = KS[0]
            current_log_info['attribute_vector'][attribute]['metric_level'] = 1 - KS[1]
            current_log_info['attribute_vector'][attribute]['actual_distribution'] = baseline_distr
            current_log_info['attribute_vector'][attribute]['interaction_distribution'] = weight_distr[attribute]
...
if __name__ == '__main__':
    ...
    #all_users = ['1506629987658']
    #all_users = ['1508339795840']
    ...
    for i in range(0, len(all_users)):
        ...
        max_weight = 1.0
        rolling_dist = 50
        attr_weight_rolling_dist = 5
        plot_svm = False
        
        for j in range(0, len(bias_util.window_methods)):
            # emily todo: change back window methods to full list
            ...
            fig_num = 2
            cur_bias.simulate_bias_computation(cur_dir + 'plots/' + window_method + '/', time, interaction_types, min_weight, max_weight, window_method, rolling_dist, attr_weight_rolling_dist, bias_util.marks, fig_num, bias_util.verbose) # fig_num 13 - 18
            print '------------------------------------------------------'